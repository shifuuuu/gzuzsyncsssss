...
ui_new_label("LUA", "B", "-----------[GzuzSync]-----------")
local master_switch = ui_new_checkbox("LUA", "B", "Enable")
local master_box = ui_new_combobox("LUA", "B", "config", "AA", "DT ENHANCEMENT", "VISUALS")
local override_antiaim = ui_new_checkbox("LUA", "B", "Enable ANTIAIM")
--local reset_stages = ui_new_hotkey("AA", "Anti-aimbot angles", "Reset bruteforce stages")
local adaptive_fs = ui_new_checkbox("LUA", "B", "APDATIVE FREESTAND")
local manual_aa = ui_new_checkbox("LUA", "B", "[M] MANUAL AA")
local manual_aa_clr = ui_new_color_picker("LUA", "B", "[M] MANUAL AA", 255, 255, 255, 255)
local manual_left = ui_new_hotkey("LUA", "B", "[M]Left")
local manual_right = ui_new_hotkey("LUA", "B", "[M]Right")
local manual_state = ui_new_slider("LUA", "B", "Manual direction", 0, 2, 0)
local faster_dt = ui_new_checkbox("LUA", "B", "RAPID DOUBLETAP")
local adaptive_dt = ui_new_checkbox("LUA", "B", "DYNAMIC SHIFT")
local legit_aa_use = ui_new_checkbox("LUA", "B", "LEGIT AA ON USE")
local low_deltakey = ui_new_hotkey("LUA", "B", "LOW DELTA ON KEY")
local first_gradient = ui_new_label("LUA", "B", "first gradient")
local primary_gradient = ui_new_color_picker("LUA", "B", "primary gradient", 255, 255, 255, 255)
local sec_gradient = ui_new_label("LUA", "B", "second gradient")
local secondary_gradient = ui_new_color_picker("LUA", "B", "secondary gradient", 0, 0, 0, 0)
local side_arrows = ui_new_checkbox("LUA", "B", "side arrows")
local side_arrows_clr = ui_new_color_picker("LUA", "B", "Side arrows color", 255, 255, 255, 255)
local GzuzSync_clr = ui_new_label("LUA", "B", "GzuzSync color")
local shandy_yaw_clr = ui_new_color_picker("LUA", "B", "GzuzSync color", 255, 255, 255, 255)
local hide_clr = ui_new_label("LUA", "B", "hideshot color")
local hide_color = ui_new_color_picker("LUA", "B", "Hide shot indicator color", 255, 255, 255, 255)
ui_new_label("LUA", "B", "-----------[GzuzSync]-----------")
...
local function paint_ui()
    ...
    if ui.get(master_box) == "AA" and ui.get(master_switch) then
    ...
    if ui.get(master_box) == "DT ENHANCEMENT" and ui.get(master_switch) then
    ...
    if ui.get(master_box) == "VISUALS" and ui.get(master_switch) then
        ...
        ui_set_visible(hide_color, true)
        ui_set_visible(hide_clr, true)
        ui_set_visible(GzuzSync_clr, true)
        ...
        ui_set_visible(hide_color, false)
        ui_set_visible(hide_clr, false)
        ui_set_visible(GzuzSync_clr, false)
...
local function manual_aa_paint()
    ...
    if m_state == 1 then renderer.text(x/2 - 40, y / 2 - 2, r, g, b, 255, "+c", 0, "⯇") end
    if m_state == 2 then renderer.text(x/2 + 40, y / 2 - 2, r, g, b, 255, "+c", 0, "⯈") end
...
local function set_anti_aim(type)
    ...
    elseif type == "right" then
        ...
        ui_set(ref_bodyyawadd, -180)
        ui_set(ref_fs_bodyyaw, false)
        ui_set(ref_fakelimit, 30)
    elseif type == "safe" then
        ui_set(ref_yaw, "180")
        ui_set(ref_yawj, "Off")
        ui_set(ref_yawjadd, 15)
        ui_set(ref_bodyyaw, "Static")
        ui_set(ref_bodyyawadd, 180)
        ui_set(ref_fs_bodyyaw, false)
        ui_set(ref_fakelimit, 25)
    ...
    elseif type == "jittervisL" then
        ...
        ui_set(ref_bodyyawadd, 180)
        ui_set(ref_fs_bodyyaw, false)
		ui_set(ref_fakelimit, math.random(5, 25))
...
local function on_paint()
    ...
    renderer.text(w / 2 + 15, h / 2 + 40, r2, g2, b2, 255, "cb", 0, "SYNC")
    ...
    local doubletap = { ui.reference("rage", "Other", "Double tap") }
        ...
        if ui.get(doubletap[1]) then
            if not ui.get(doubletap[2]) then
                renderer.text(center_x, center_y+50, 255, 0, 0, 255, "cb", 0, "DT")
            end
        end
        ...
        if ui.get(doubletap[2]) then
            ...
            if math.max(nextShot,nextShotSecondary) < nextAttack then -- swapping
                if nextAttack - globals.curtime() > 0.00 then
                    ...
                    renderer.text(center_x, center_y+50, 150, 200, 60, 255, "cb", 0, "DT")
    
                end
            else -- shooting or just shot
                if math.max(nextShot,nextShotSecondary) - globals.curtime() > 0.00  then
                    renderer.text(center_x, center_y+50, 255, 0, 0, alpha, "cb", 0, "DT")
                    renderer.circle_outline(center_x+12, center_y+51, 150, 200, 60, 255, 4, 270, 1.0 - (3 * math.abs(math.max(nextShot,nextShotSecondary) - globals.curtime())), 2)
                    recharging = true
                else
                    recharging = false
                    if math.max(nextShot,nextShotSecondary) - globals.curtime() < 0.00  then
...
local function draw()
    ...
    renderer.text(w / 2 - 12, h / 2 + 40, m_r, m_g, m_b, 255, "cb", 0, "GZUZ ")
    		-- draw gif at top right corner of the menu


    local h_r, h_g, h_b = ui.get(hide_color)
    if ui.get(ref_hs_key) and ui.get(ref_hs) == true then
        renderer.text(w / 2, h / 2 + 60, h_r, h_g, h_b, 255, "cb", 0, "HIDE")
    else
        renderer.text(w / 2, h / 2 + 60, h_r, h_g, h_b, 255, "cb", 0, " ")
    end
        ...
        if not ui_get(manual_aa) or ui_get(manual_state) == 0 then
            if brute.best_angle ~= 0 or brute.indexed_angle ~= 0 then
                ...
                if brute.yaw_status == "left" then
                    renderer.text(w / 2 + 40, h / 2 - 2, 255, 255, 255, 150, "cb+", 0, ">")
                    renderer.text(w / 2 - 40, h / 2 - 2, s_r, s_g, s_b, s_a, "cb+", 0, "<")
                elseif brute.yaw_status == "right" then
                    renderer.text(w / 2 + 40, h / 2 - 2, s_r, s_g, s_b, s_a, "cb+", 0, ">")
                    renderer.text(w / 2 - 40, h / 2 - 2, 255, 255, 255, 150, "cb+", 0, "<")
                elseif brute.yaw_status == "jitter" then
